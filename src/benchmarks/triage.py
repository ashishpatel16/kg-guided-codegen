...
class TriageEnvironment:
    ...
    def setup_workspace(self, work_root: str = None) -> str:
        ...
        if work_root is None:
            ...
            assert os.path.exists(self.work_dir), f"Working directory {self.work_dir} does not exist"
    ...
    def setup_environment(self) -> str:
        ...
        subprocess.run([self.pip_path, "install", "--upgrade", "pip", "setuptools", "wheel"], check=True)
    ...
    def _install_dependencies(self):
        ...
        if os.path.exists(req_path):
            ...
            try:
                ...
                logger.warning("Failed to install some requirements from requirements.txt")

        # 2. Try setup.py or pyproject.toml for editable install
        if os.path.exists(os.path.join(self.repo_dir, "setup.py")) or \
           os.path.exists(os.path.join(self.repo_dir, "pyproject.toml")):
            ...
            try:
                subprocess.run([self.pip_path, "install", "-e", self.repo_dir], check=True)
            except subprocess.CalledProcessError:
        ...
        if self.version:
            try:
                ...
                subprocess.run([self.pip_path, "install", f"{package_name}=={self.version}"], check=True)
    ...
    def _verify_installation(self):
        ...
        try:
            subprocess.run(
                [self.python_path, "-c", f"import {package_name}; print('Import successful')"], 
                check=True, capture_output=True
            )
            logger.info("Package import successful.")
        except subprocess.CalledProcessError:
            logger.warning(f"Could not import {package_name}. The package name might differ from the repo name or installation failed.")
